...
- **Dashboard** — Memory totals including raw vs consolidated layer counts, breakdowns by type/category/layer/role (Chart.js donut charts), a scrollable right-side legend for the category chart, operation counts, per-user filtering, server version display, auto-refresh with manual refresh button
...
- **Graph** — D3.js force-directed visualization of memory relationships (shared categories = edges, node size = importance, color = memory type). Type toggles plus memory-layer and category filters, node limit slider, click-to-select detail panel
- **User** — Default core-context preview matching `get_core_memories`, including agent-scoped view when an agent is selected. Shows approximate token usage and compact stats for the included memories, plus an advanced mode that loads the underlying memories as interactive cards for cleanup/editing/artifacts
- **Check** — Memory consistency checker (fsck). Run a three-phase scan (security, duplicates, quality) against durable memories by default, review issues grouped by type with full reasoning and affected memory cards, filter by severity/confidence, edit or delete affected memories directly, and apply selected fixes in bulk. Includes live progress tracking plus an auto-fsck banner with Run Now / View Last Results shortcuts
- **Sessions** — Browse persistent session summaries from the remember endpoint with server-side pagination, summary text search, sorting by last activity or creation time, page size selection, and consolidation-state filtering (idle, consolidating, consolidated, failed). Expandable cards with full summary, metadata, linked memory cards with full CRUD actions, and consolidated memory results. "Consolidate Now" button for idle/failed sessions. Superseded badges on raw memories, consolidated memory section with green accent
