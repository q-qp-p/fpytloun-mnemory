...
- **Dashboard** — Memory totals including raw vs consolidated layer counts, breakdowns by type/category/layer/role (Chart.js donut charts), operation counts, per-user filtering, server version display, auto-refresh with manual refresh button
